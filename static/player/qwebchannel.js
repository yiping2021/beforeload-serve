...
var QWebChannel = function(transport, initCallback, logger) {
  ...
  this.exec = function(data, callback) {
    ...
    if (this.logger && this.inited) {
      if (data.type == QWebChannelMessageTypes.invokeMethod) {
        var uri = this.objects[data.object].objectName + '.' + data.method
        this.logger.onRequest(data.id, uri, data.args)
      }
      //TODO: else ?
    }
  ...
  this.handleResponse = function(message) {
    ...
    if (this.logger && this.inited) {
      this.logger.onResponse(message.id, message.data)
    }
...
function QObject(name, data, webChannel) {
  ...
  function addSignal(signalData, isPropertyNotifySignal) {
    ...
    object[signalName] = {
      connect: function(callback) {
        ...
        object.__signaleNames__ = object.__signaleNames__ || {}
        object.__signaleNames__[signalIndex] = signalName
        object.__objectSignals__[signalIndex] = object.__objectSignals__[signalIndex] || []
        object.__objectSignals__[signalIndex].push(callback)

        if (webChannel.logger && webChannel.inited) {
          var fullName = webChannel.objects[object.__id__].objectName + '.' + signalName
          webChannel.logger.onConnectSignal(
            object.__objectSignals__[signalIndex].length - 1,
            fullName,
            callback
          )
        }
      ...
      disconnect: function(callback) {
        ...
        if (webChannel.logger && webChannel.inited) {
          var fullName = webChannel.objects[object.__id__].objectName + '.' + signalName
          webChannel.logger.onDisconnectSignal(idx, fullName, callback)
        }
  ...
  this.signalEmitted = function(signalName, signalArgs) {
    if (webChannel.logger && webChannel.inited) {
      var fullName =
        webChannel.objects[object.__id__].objectName + '.' + object.__signaleNames__[signalName]
      webChannel.logger.onSignal(fullName, signalArgs)
    }
